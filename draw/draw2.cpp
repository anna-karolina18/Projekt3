...
void wpis(int ktory)
{
	if (ktory == 1)
	{
		plik.open("outputRobotForwardB01.log", ios::in);
	}
	else if(ktory==2)
	{
...
{
	int i = 0, odrz = 25;
	double pred = 0, drog = 0, suma = 0; 

	for (odrz = 25; odrz < ile; odrz++)
	{
		suma = 0;
		for (i = odrz - 25; i < odrz; i++)
		{
			suma = suma + tab[i] * 9.81;
		}
		srednia[odrz] = suma / 25;
	}
	for (i = 25; i < ile; i++)
...
void RysPredkosc(HDC hdc , int y , int t)
{
	...
	int ilep = ile / t + 1;
	int pocz = 24 / t + 1;
	MoveToEx(hdc,pocz + 20, 385, &stary_punkt);
	for (int i = pocz+1; i < ilep; i++)
	{
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	hwndButton = CreateWindow(TEXT("button"), TEXT("s"),                  
		WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,  
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_BUTTON1 :
			...
			droga = new double[ile];
			srednia = new double[ile];
			wpis2();
